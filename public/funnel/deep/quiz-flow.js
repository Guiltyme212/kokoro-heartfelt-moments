...
/* ---------- DEEP NEW: comparison bars — kept-overthinking vs people like you ---------- */
R.compare = s => {
  const bar = (d, lead) => '<div class="cmp-row t-'+d.tone+(lead?' is-lead':'')+'">'+
    '<div class="cmp-top"><span class="cmp-lbl">'+d.label+'</span>'+
      '<span class="cmp-val" data-to="'+d.value+'">0%</span></div>'+
    '<div class="cmp-bar"><i style="width:0%" data-w="'+d.value+'"></i></div></div>';
  return panel(miniback()+'<div class="qbody"><div class="stmt-eye">'+s.eyebrow+'</div>'+
    '<h2 class="qtitle">'+s.title+'</h2>'+
    '<div class="cmp-card"><div class="cmp-metric">'+s.metric+'</div>'+
      bar(s.a, false)+bar(s.b, true)+
    '</div>'+
    '<p class="chart-p">'+s.p+'</p>'+
    '<div class="chart-note">'+svg('feather',14)+'<span>'+s.note+'</span></div>'+
    '<div style="flex:1;min-height:10px"></div></div>'+
...
R.wire_compare = (s, root) => {
  const rows = [...root.querySelectorAll('.cmp-row')];
  const run = ()=> rows.forEach((row,idx)=>{
    const bar = row.querySelector('.cmp-bar i');
    const val = row.querySelector('.cmp-val');
    const to = +bar.dataset.w;
    setTimeout(()=>{ bar.style.width = to+'%'; }, 120 + idx*180);
    if(val){
      const dur=1100, t0=performance.now();
      const step=(t)=>{ const k=Math.min(1,(t-t0)/dur), e=1-Math.pow(1-k,3);
        val.textContent=Math.round(to*e)+'%'; if(k<1) requestAnimationFrame(step); };
      setTimeout(()=>requestAnimationFrame(step), 200 + idx*180);
    }
  });
...
/* DEEP — premium pricing (no free trial). Index matches the paywall
   plans: 0 = yearly €69.99/yr, 1 = monthly €22.99/mo, 2 = weekly €8.99/wk.
   Each link's success_url should point at
   thanks.html?plan=<name>&value=<value> so Purchase fires. */
const STRIPE_LINKS = [
  'https://buy.stripe.com/eVq7sK1Y1cx2fXYeLw5os0g', // yearly  €69.99/yr
  'https://buy.stripe.com/9B66oG8mp7cI4fggTE5os0h', // monthly €22.99/mo
  'https://buy.stripe.com/eVq14mfOR0Ok3bc7j45os0i'  // weekly  €8.99/wk
];
/* Meta Pixel — safe no-op if blocked/not loaded */
function fbtrack(ev, params){ try{ if(window.fbq) fbq('track', ev, params||{}); }catch(e){} }

const PLAN_VALUE = [69.99, 22.99, 8.99];   // yearly / monthly / weekly (EUR)
  ...
  const fireLead = ()=>{
    ...
    if(EMAIL_ENDPOINT){                           // save to the Google Sheet (fire-and-forget)
      try{ fetch(EMAIL_ENDPOINT, { method:'POST', mode:'no-cors',
        ...
        body: JSON.stringify({ email:e, plan: PLAN_NAME[state.plan]||'', source:'funnel-deep', ts: Date.now() }) }); }catch(_){}
    }
  };
