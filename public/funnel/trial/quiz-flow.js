...
const STRIPE_LINKS = [
  'https://buy.stripe.com/cNi28q7ileFa6no1YK5os0f', // 7-day free trial -> €19.99/mo
  'https://buy.stripe.com/7sY8wO0TX1So8vw9rc5os0e'  // yearly €59.99/yr
];
/* Meta Pixel — safe no-op if blocked/not loaded */
function fbtrack(ev, params){ try{ if(window.fbq) fbq('track', ev, params||{}); }catch(e){} }

const PLAN_VALUE = [19.99, 59.99];   // 7-day free trial (-> €19.99/mo) / yearly (EUR)
const PLAN_NAME  = ['trial','yearly'];
  ...
  const fireLead = ()=>{
    ...
    if(EMAIL_ENDPOINT){                           // save to the Google Sheet (fire-and-forget)
      try{ fetch(EMAIL_ENDPOINT, { method:'POST', mode:'no-cors',
        ...
        body: JSON.stringify({ email:e, plan: PLAN_NAME[state.plan]||'', source:'funnel-trial', ts: Date.now() }) }); }catch(_){}
    }
  };
