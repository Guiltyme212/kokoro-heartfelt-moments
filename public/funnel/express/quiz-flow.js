...
/* V1 STANDARD — pricing strategy #2 (no free trial). Index matches the paywall
   plans: 0 = annual €29.99/yr, 1 = monthly €9.99/mo. Apple Pay shows one-tap.
   TODO(stripe): replace these with the real €29.99/yr + €9.99/mo payment links
   created via the Stripe API, and set each link's success_url to
   thanks.html?plan=<name>&value=<value> so Purchase fires. */
const STRIPE_LINKS = [
  'https://buy.stripe.com/eVq5kCdGJ54A8vwcDo5os0b', // yearly  €59.99/yr
  'https://buy.stripe.com/14A9ASbyBaoU6nobzk5os0c', // monthly €19.99/mo
  'https://buy.stripe.com/4gM6oG46968E6nobzk5os0d'  // weekly  €7.99/wk
];
/* Meta Pixel — safe no-op if blocked/not loaded */
function fbtrack(ev, params){ try{ if(window.fbq) fbq('track', ev, params||{}); }catch(e){} }

const PLAN_VALUE = [59.99, 19.99, 7.99];   // yearly / monthly / weekly (EUR)
  ...
  const fireLead = ()=>{
    ...
    if(EMAIL_ENDPOINT){                           // save to the Google Sheet (fire-and-forget)
      try{ fetch(EMAIL_ENDPOINT, { method:'POST', mode:'no-cors',
        ...
        body: JSON.stringify({ email:e, plan: PLAN_NAME[state.plan]||'', source:'funnel-express', ts: Date.now() }) }); }catch(_){}
    }
  };
