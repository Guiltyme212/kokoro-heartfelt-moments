...
/* "almost-zero" per-day price card \u2014 Playa-style formula, kokoro skin.
   Each plan's right side is a giant integer + tiny decimals + "per day". */
R.paywall = s => {
  const sel = Math.max(0, s.plans.findIndex(p=>p.sel));   // pre-selected row (popular)
  const plans = s.plans.map((p,i)=>
    '<div class="pplan'+(i===sel?' is-on':'')+(p.popular?' pop':'')+'" data-i="'+i+'">'+
      (p.popular?'<span class="pop-rib">most popular</span>':'')+
      '<div class="pp-main">'+
        '<div class="pp-radio"></div>'+
        '<div class="pp-left">'+
          '<div class="pp-name">'+p.name+(p.off?'<span class="pp-off">'+p.off+'</span>':'')+'</div>'+
          '<div class="pp-was">'+(p.anchor?'<s>'+p.anchor+'</s> ':'')+'<b>'+p.price+'</b> total</div>'+
        '</div>'+
        '<div class="pp-day"><span class="ppd-cur">'+p.cur+'</span><span class="ppd-big">'+p.big+'</span>'+
          '<span class="ppd-tail"><span class="ppd-dec">'+p.dec+'</span><span class="ppd-per">per day</span></span></div>'+
      '</div>'+
    '</div>').join('');
  const promo = s.promo ? '<div class="promo">'+
      '<span class="promo-badge">'+s.promo.pct+'</span>'+
      '<div class="promo-mid"><div class="promo-t">'+s.promo.title+'</div><div class="promo-s">'+s.promo.sub+'</div>'+
        '<div class="promo-code"><span class="promo-ck">'+I_CHECK+'</span>'+s.promo.code+'</div></div>'+
      '<b class="promo-clock" data-clock>10:00</b>'+
    '</div>' : '';
  ...
  return panel(miniback()+'<div class="qbody pw pw1">'+
    '<div class="pw-expiry"><span>'+s.expiry+'</span><b data-clock>10:00</b></div>'+
    promo+
    '<h2 class="pw1-h">'+s.heading+'</h2>'+
    '<div class="pplans">'+plans+'</div>'+
    (s.proof?'<div class="pw1-proof">'+svg('star',14)+'<span>'+s.proof+'</span></div>':'')+
    cards+
    ...
    '<div class="qfoot"><button class="cta cta-shine" data-pay="1">get my result &amp; plan</button></div>');
};
...
/* VALUE funnel — premium price + hard yearly push + aggressive anchoring.
   ...
   Purchase fires. */
/* Order matches the on-screen plan rows in this variant: week · month · year.
   state.plan is the row index, so these three arrays line up with quiz-data plans. */
const STRIPE_LINKS = [
  'https://buy.stripe.com/14AdR8cCF1So4fg1YK5os0l', // 1-week   €9.99/wk
  'https://buy.stripe.com/7sY28qauxgNibHI6f05os0k', // 4-week   €24.99/mo
  'https://buy.stripe.com/fZu28q0TXaoU6nodHs5os0j'  // 12-month €79.99/yr
];
...
const PLAN_VALUE = [9.99, 24.99, 79.99];   // week / month / year (EUR)
const PLAN_NAME  = ['weekly','monthly','yearly'];
...
R.wire_paywall = (s, root) => {
  const ctas = [...root.querySelectorAll('[data-pay]')];
  const renew = root.querySelector('[data-renew]');
  const setRenew = ()=>{ const p=s.plans[state.plan]; if(!renew) return;
    const isWk=/week/.test(p.name), isYr=/year|12[\s-]*month/.test(p.name);
    renew.textContent = 'then '+p.price+(isWk?' / week, renews weekly':isYr?' / year, renews annually':' / 4 weeks, renews monthly')+' · cancel anytime'; };
  // pre-select the popular plan, so state.plan is correct before any tap
  state.plan = Math.max(0, s.plans.findIndex(p=>p.sel));
  root.querySelectorAll('.pplan').forEach(p=>{
    p.onclick = ()=>{
      root.querySelectorAll('.pplan').forEach(x=>x.classList.remove('is-on'));
      p.classList.add('is-on'); state.plan=+p.dataset.i; setRenew();
    };
  });
  setRenew();
  ctas.forEach(c=> c.addEventListener('click', goToCheckout));
  /* a real, per-session countdown (drives both the expiry bar and promo clock) */
  const clocks = [...root.querySelectorAll('[data-clock]')];
  if(clocks.length){
    let left = (s.countdown || 600);
    const tick = ()=>{ const m=Math.floor(left/60), ss=left%60; const t=m+':'+(ss<10?'0':'')+ss;
      clocks.forEach(c=> c.textContent = t); if(left>0) left--; };
    tick(); const iv = setInterval(()=>{ if(!clocks[0].isConnected){ clearInterval(iv); return; } tick(); }, 1000);
  }
};
  ...
  const fireLead = ()=>{
    ...
    if(EMAIL_ENDPOINT){                           // save to the Google Sheet (fire-and-forget)
      try{ fetch(EMAIL_ENDPOINT, { method:'POST', mode:'no-cors',
        ...
        body: JSON.stringify({ email:e, plan: PLAN_NAME[state.plan]||'', source:'funnel-1', ts: Date.now() }) }); }catch(_){}
    }
  };
