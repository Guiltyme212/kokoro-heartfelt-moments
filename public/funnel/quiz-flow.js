...
STEPS.forEach((s,i)=>{
  s._pct = i<=FIRST ? 8 : i>=LAST ? 96 : Math.round(8 + (i-FIRST)/(LAST-FIRST)*86);
});
...
R.paywall = s => {
  const plans = s.plans.map((p,i)=>'<div class="plan'+(i===0?' is-on':'')+'" data-i="'+i+'">'+(p.best?'<span class="besttag">best value</span>':'')+
    '<div class="radio"></div><div class="pn"><b>'+p.name+'</b><span>'+p.note+'</span></div><div class="pp">'+p.price+'<small>'+p.per+'</small></div></div>').join('');
  const nm = state.name ? state.name+'\u2019s' : 'your';
  return panel(miniback()+'<div class="qbody pw"><div class="pw-disc"><img src="'+aset('meditatePng','assets/kokoro-meditate.png')+'" alt="Kokoro"/></div>'+
...
R.wire_loading = (s, root) => {
  ...
  function adv(){
    ...
    else setTimeout(next, 720);
  }
  function ask(){
    main.style.display='none'; intr.hidden=false; intr.classList.add('anim');
    intr.querySelectorAll('.opt').forEach(o=> o.onclick = ()=>{
      ...
      o.classList.add('is-on');
      setTimeout(()=>{ intr.hidden=true; main.style.display=''; setTimeout(adv, 360); }, 460);
    });
  }
  adv();
};
...
function goToCheckout(){
  ...
  fbtrack('InitiateCheckout', { value: PLAN_VALUE[state.plan] || 7, currency: 'EUR',
                                content_name: PLAN_NAME[state.plan] || 'trial' });
...
R.wire_paywall = (s, root) => {
  ...
  root.querySelectorAll('.plan').forEach(p=>{
    p.onclick = ()=>{
      ...
      p.classList.add('is-on'); state.plan=+p.dataset.i;
